...
def _main(args, output_file):
    logging.basicConfig(
        ...
        datefmt='%Y-%m-%d %H:%M:%S',
        level=logging.INFO if args.distributed_rank == 0 else logging.WARN,
        stream=output_file,
    )
    ...
    model_overrides['data'] = args.data   # need to fix the data path
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for i, sample in enumerate(t):        
            ...
            if (args.render_maximum_frames is not None) and \
                (step > args.render_maximum_frames):
                break
    ...
    # try:
    #     samples = distributed_utils.all_gather_list(sample)
    # except:
    #     timestamps = [sample]
...
def cli_main():
    ...
    print (f'DEBUG:: render_multigpu: cli_main: args:', args)
