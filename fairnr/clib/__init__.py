...
class TriangleRayIntersect(Function):
    ...
    def forward(ctx, cagesize, blur_ratio, n_max, points, faces, ray_start, ray_dir):
        # Organize rays in batches to speedup computations in GPU

        ## TODO: when the batch size is already large, or the number of points is very small,
        # (as in our patch-based training strategy), for some reason this function is
        # making every point into a unique batch, which explodes in memory usage.
        G = min(2048, int(2 * 10 ** 9 / (3 * faces.numel()))) # Avoid out-of-memory
        S, N = ray_start.shape[:2]
        K = int(np.ceil(N / G))
        H = K * G
        if H > N:
            # with dummy data in the right amount.
            ray_dummy = -1 * torch.ones((S, H - N, 3), dtype=ray_start.dtype).to(ray_start.device)
            ray_start = torch.cat([ray_start, ray_dummy], 1)
            ray_dir = torch.cat([ray_dir, ray_dummy], 1)
