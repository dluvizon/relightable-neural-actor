...
class RenderingCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        #print (f'DEBUG:: forward: sample:', sample.keys())
        #print (f'DEBUG:: forward: sample size:', sample['size'])
...
class SRNLossCriterion(RenderingCriterion):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--color-weight-start', type=int, default=0)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        #print (f'DEBUG:: in compute_loss')
        #print (f'\n\nDEBUG:: sample', sample.keys())
        #print (f'DEBUG:: sample path', sample['path'])
    
        # prepare data before computing loss
        sampled_uv = sample['sampled_uv'].float()  # S, V, 2, N, P, P (patch-size)
        #print ('DEBUG:: sampled_uv', sampled_uv.shape, '\t', sampled_uv.min(), sampled_uv.max())
        #print ('DEBUG:: sampled_uv.shape', sampled_uv.shape)
        #print ('DEBUG:: sampled_uv.min/max', sampled_uv.min(), sampled_uv.max())

        S, V, _, N, P1, P2 = sampled_uv.size()
        #print (f'DEBUG:: sample size::', sample['size'])

        #H, W, h, w = sample['size'][0, 0].long().cpu().tolist()
        H, W, ratio_h, ratio_w = sample['size'][0, 0].cpu().tolist()
        # h = H * ratio_h
        # w = W * ratio_w

        #print (f'DEBUG:: H, W, h, w, ratio_h, ratio_w', H, W, h, w, ratio_h, ratio_w) # -->> h is ZERO!!!
        L = N * P1 * P2
        flatten_uv = sampled_uv.view(S, V, 2, L)
        #print ('DEBUG:: flatten_uv', flatten_uv.shape, '\t', flatten_uv.min(), flatten_uv.max())
        #print ('DEBUG:: flatten_uv.min/max', flatten_uv.min(), sampled_uv.max())

        #flatten_index = (flatten_uv[:,:,0] // h + flatten_uv[:,:,1] // w * W).long()
        # x_coords = flatten_uv[:,:,0]
        # y_coords = flatten_uv[:,:,1]
        # print ('DEBUG:: x_coords ', x_coords.shape, '\t', x_coords.min(), x_coords.max())
        # print ('DEBUG:: y_coords ', y_coords.shape, '\t', y_coords.min(), y_coords.max())

        # x_coords /= ratio_w
        # y_coords /= ratio_h
        # print ('DEBUG:: x_coords~', x_coords.shape, '\t', x_coords.min(), x_coords.max())
        # print ('DEBUG:: y_coords~', y_coords.shape, '\t', y_coords.min(), y_coords.max())
        flatten_index = (flatten_uv[:,:,0] / ratio_w).long() + ((flatten_uv[:,:,1] / ratio_h) * W).long()
        # print (f'DEBUG:: flatten_index', flatten_index.shape, flatten_index.min(), flatten_index.max())
        ...
        #print (f'DEBUG:: here comes the shit....')

        #print (f'DEBUG:: target_colors', target_colors)
        #print (f'DEBUG:: masks', masks)
        #print (f'DEBUG:: flatten_uv', flatten_uv)

        #print (f'DEBUG:: target_colors', target_colors.shape, target_colors.min(), target_colors.max())
        #print (f'DEBUG:: masks.shape', masks.shape)

        if L < target_colors.size(2):
            aux = flatten_index.unsqueeze(-1).repeat(1,1,1,3)
            #print (f'DEBUG:: aux.shape', aux.shape)
            target_colors = target_colors.gather(2, aux)
        ...
        if self.args.ssim_weight > 0 and self.training:
            import pytorch_msssim
            assert P1 * P2 > 1, "we have to use a patch-based sampling for mincolor loss"
            true = target_colors.reshape(-1, P1, P2, 3).permute(0, 3, 1, 2) * .5 + .5
            pred = net_output['colors'].reshape(-1, P1, P2, 3).permute(0, 3, 1, 2) * .5 + .5

            ssim_loss = pytorch_msssim.ssim(true, pred, window_size=3)
            ssim_loss = torch.relu(ssim_loss)

            losses['msssim_loss'] = (-ssim_loss, self.args.ssim_weight)
        
        if (self.args.alpha_weight > 0) and hasattr(net_output, 'missed'):
        ...
        if "additional_loss" in net_output:
            for loss in net_output["additional_loss"]:
                losses[loss['name'] + "_loss"] = (loss['loss'], loss['factor'])
        ...
        #print (f'DEBUG:: done in compute_loss')
