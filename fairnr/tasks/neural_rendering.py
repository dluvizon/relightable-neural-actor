...
from fairnr.data import (
    ...
    WorldCoordDataset, ShapeDataset, InfiniteDataset, VideoDataset
)
from fairnr.data.data_utils import write_images, recover_image, parse_views, GPUTimer
...
class SingleObjRenderingTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--load-all-images", action="store_true",
                            help="load all images for each view")
        ...
        parser.add_argument("--subsample-train", type=int, default=-1,
                            help="if set > -1, subsample the validation (when training set is too large)")
        parser.add_argument("--start-end", type=str, default=None,
                            help="example say, (100,200)")
        ...
        parser.add_argument("--load-rgb-name", type=str, default="masked_rgb")

        parser.add_argument('--load-video-dataset', action='store_true',
                            help="use video dataset instead of standard dataset")
    ...
    def __init__(self, args):
        ...
        print (f'DEBUG:: SingleObjRenderingTask:: __init__')
        print (f'DEBUG:: SingleObjRenderingTask:: __init__: args.data', args.data)

        # check dataset
        self.train_data = self.val_data = self.test_data = args.data
        if args.valid_data is not None:
            self.val_data = args.valid_data
        ...
        if (self.args.tensorboard_logdir is not None) and (len(self.args.tensorboard_logdir) > 0) and getattr(args, "distributed_rank", -1) == 0:
        ...
        print (f'DEBUG:: self.train_views', self.train_views)
        print (f'DEBUG:: self.valid_views', self.valid_views)
        print (f'DEBUG:: self.test_views', self.test_views)
    ...
    def setup_task(cls, args, **kwargs):
        ...
        Setup the task
        """
        print (f'DEBUG:: SingleObjRenderingTask:: setup_task')
        print (f'DEBUG:: SingleObjRenderingTask:: setup_task: cls', cls)
        return cls(args)
    ...
    def load_dataset(self, split, **kwargs):
        ...
        """
        print (f'DEBUG:: SingleObjRenderingTask::load_dataset: split:', split)

        Dataset = ShapeViewDataset if not getattr(self.args, "load_video_dataset", False) else VideoDataset
        print (f'DEBUG:: SingleObjRenderingTask::load_dataset, Dataset:', Dataset)

        dataset_kargs = {
            'start_end': self.args.start_end,
            ...
            'ids': self.object_ids,
            'load_all_images': getattr(self.args, "load_all_images", False),
            'load_rgb_name': getattr(self.args, "load_rgb_name", "masked_rgb")
        }

        if split == 'train':
            dataset_kargs['paths'] = self.train_data
            dataset_kargs['views'] = self.train_views
            dataset_kargs['num_view'] = self.args.view_per_batch
            dataset_kargs['subsample_valid'] = self.args.subsample_train
            dataset_kargs['resolution'] = self.args.view_resolution

        elif split == 'valid':
            dataset_kargs['paths'] = self.val_data
            dataset_kargs['views'] = self.valid_views
            dataset_kargs['num_view'] = self.args.valid_view_per_batch
            dataset_kargs['subsample_valid'] = self.args.subsample_valid
            dataset_kargs['resolution'] = getattr(self.args, "valid_view_resolution", self.args.view_resolution)

        elif split == 'test':
            dataset_kargs['paths'] = self.test_data
            dataset_kargs['views'] = self.test_views
            dataset_kargs['num_view'] = 1
            dataset_kargs['subsample_valid'] = self.args.subsample_valid
            dataset_kargs['resolution'] = getattr(self.args, "render_resolution", self.args.view_resolution)

        self.datasets[split] = Dataset(**dataset_kargs)
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, ignore_grad=False):
        # print (f'\t\tDEBUG:: SingleObjRenderingTask::train_step >>>>>>>>')
        if (((self.pruning_every_steps is not None) and \
            ...
            # print (f'\t\tDEBUG:: Warn: pruning_every_steps')
            model.eval()
            if getattr(self.args, "pruning_rerun_train_set", False):
                with torch.no_grad():
        ...
        if self.steps_to_half_voxels is not None and \
            ...
            # print (f'\t\tDEBUG:: Warn: steps_to_half_voxels')
            model.split_voxels()
        ...
        if self.rendering_every_steps is not None and \
            ...
            # print (f'\t\tDEBUG:: Warn: rendering_every_steps')
        ...
        if self.steps_to_reduce_step is not None and \
            ...
            # print (f'\t\tDEBUG:: Warn: steps_to_reduce_step')
    ...
    def valid_step(self, sample, model, criterion):
        with GPUTimer() as timer:
            ...
            if (self.writer is not None) and (self.output_valid is None):
                images = model.visualize(sample, shape=0, view=0)
                if images is not None:
                    write_images(self.writer, images, self._num_updates['step'])
        # logger.info("valid step..., total time={:.4f}".format(timer.sum))
        return loss, sample_size, logging_output


    def marching_cubes(self, sample, model, fname):
        output_path = os.path.join(self.output_valid, 'mc_mesh')
        os.makedirs(output_path, exist_ok=True)
        with open(os.path.join(output_path, fname), 'wb') as fd:
            with GPUTimer() as timer:
                encoder_states = model.preprocessing(**sample)
                plydata = model.encoder.export_surfaces(model.field.forward, 50, encoder_states=encoder_states)
                # plydata = model.export_surfaces(**sample)
            # plydata.text = True
            plydata.write(fd)
