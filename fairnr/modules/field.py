...
class RaidanceField(Field):
    ...
    def __init__(self, args):
        ...
        self.pe_no_mul_pi = getattr(args, "pe_no_mul_pi", False)
        ...
        self.total_experts = getattr(args, "total_experts", 0)
        self.num_experts = getattr(args, "num_experts", 0)
    ...
    def build_feature_field(self, args): 
        ...
        if not getattr(args, "hypernetwork", False):
            #self.implicit_field_block = ImplicitField( # NEURA TODO
    ...
    def build_density_predictor(self, args):
        den_feat_dim = self.tex_input_dims[0]
        #self.density_head_block = SignedDistanceField( # NEURA TODO
    ...
    def build_texture_renderer(self, args):
        ...
        #self.texture_head_block = TextureField( # NEURA TODO
    ...
    def add_args(parser):
        ...
        parser.add_argument('--inputs-to-texture', type=str,
                            help="""
                                ...
                                """)
        parser.add_argument('--pe-no-mul-pi', action='store_true',
                            help="not having PI in positional encoding.")
        ...
        parser.add_argument('--total-experts', type=int, default=0)
    ...
    def forward(self, inputs, outputs=['sigma', 'texture']):
        ...
        # Arguments
            inputs: dictionary with
                'ray': tensor, 3D points (num_rays, 3)
                'dists': tensor, distances from last/next points ?? (num_rays,)
                'texture': tensor, sampled texture features (num_rays, 512)
                'attnout': tensor, positional encoding of local u,v,h coords (num_rays, 256)
                'weights': tensor, blending weights of the sampled 3D points (num_rays, 24) 
                'joint_colors': tensor, sampled color at joints locations ?? TODO check that (num_rays, 4)
                'posed_pos': tensor, sampled points in the normalized posed space (num_rays, 3)
                'pos': tensor, sampled points in the cannonical space (num_raus, 3)
            outputs: list of strings defining the desired outputs

        # Returns
            A dictionary with the following fields added to the inputs:
                'feat': tensor, features computed from position encoding ?? (num_rays, 256)
                'texture': tensor (num_rays, 512), if 'texture' in outputs
                'sigma': tensor, (num_rays,), if 'sigma' in outputs
        """

        if self.total_experts > 1:
            assert "weights" in inputs, "need to have weights for each joint"
            topk_weights, topk_indices = inputs['weights'].topk(self.num_experts, dim=-1)
            topk_weights = topk_weights / topk_weights.sum(-1, keepdim=True)
            topk_indices = topk_indices.reshape(-1)
            topk_indices, topk_ranks = topk_indices.sort(-1)
            topk_sizes = [(topk_indices == k).sum() for k in range(self.total_experts)]
        ...
        if inputs.get('feat', None) is None:        
            for i, name in enumerate(self.den_filters):
                ...
                assert (name in inputs), "the encoder must contain target inputs: {} in {}".format(name, inputs)
                ...
                if name == 'context':
                    ...
                    if (self.total_experts > 1) and (inputs[name].size(0) < topk_indices.size(0)):
                        assert inputs[name].size(0) * self.num_experts == topk_indices.size(0)
                        inputs[name] = inputs[name][:, None, :].expand(
                            inputs[name].size(0), self.num_experts, -1).reshape(
                                inputs[name].size(0) * self.num_experts, -1)
                        inputs[name] = inputs[name][topk_ranks]
            ...
            elif (self.total_experts > 1):
                filtered_inputs = (filtered_inputs, topk_sizes)
            else:
                filtered_inputs = (filtered_inputs, )
            #inputs['feat'] = self.implicit_field_block(*filtered_inputs) # (num_rays, 256) # NEURA TODO
            inputs['feat'] = self.feature_field(*filtered_inputs)

        if 'sigma' in outputs:
            assert 'feat' in inputs, "feature must be pre-computed"
            if (self.total_experts > 1):
                filtered_inputs = (inputs['feat'], topk_sizes)
            else:
                filtered_inputs = (inputs['feat'], ) # (num_rays, 256)
            #inputs['sigma'] = self.density_head_block(*filtered_inputs)[0] # NEURA TODO
            inputs['sigma'] = self.predictor(*filtered_inputs)[0]
        ...
        if ('normal' not in inputs) and (
            ...
            inputs['normal'] = -grad_pos

            # if not getattr(self.args, "no_normalize_normal", False):
            #     inputs['normal'] = F.normalize(-grad_pos, p=2, dim=1) 
            # else:
        ...
        if 'texture' in outputs:        
            ...
            filtered_inputs = torch.cat(filtered_inputs, -1) # TODO: what are all these inputs ?? feat: field from pos emb.; texture from textures, 
            if (self.total_experts > 1):
                filtered_inputs = (filtered_inputs, topk_sizes)
            else:
                filtered_inputs = (filtered_inputs, )
            
            #inputs['texture'] = self.texture_head_block(*filtered_inputs) # (num_rays, 3) # NEURA TODO
        ...
        if self.total_experts > 1:
            for name in outputs:
                flag = False
                if inputs[name].dim() == 1:
                    inputs[name] = inputs[name].unsqueeze(-1)
                    flag = True

                inputs[name] = torch.einsum("nkd,nk->nd",
                    inputs[name].clone().scatter(0, 
                        topk_ranks[:, None].expand_as(inputs[name]),
                        inputs[name]).view(-1, self.num_experts, inputs[name].size(-1)), 
                    topk_weights)
                
                if flag:
                    inputs[name] = inputs[name].squeeze(-1)
        return inputs
...
class SDFRaidanceField(RaidanceField):
    ...
    def build_feature_field(self, args):
        ...
        self.feature_field = ImplicitField(
            ...
            num_layers, with_ln=False, skips=skips, outmost_linear=True, 
            spec_init=False, use_softplus=True)  
        ...
        for l in range(num_layers):
            #lin = self.implicit_field_block.net[l]  # NEURA TODO
    ...
    def build_density_predictor(self, args):
        class Sdf2Densityv1(nn.Module):
            def __init__(self):
                ...
                # self.sigma = nn.Parameter(torch.scalar_tensor(20.0), requires_grad=True)
            
            def forward(self, x):
                return self.sigma * torch.tanh(-torch.abs(self.alpha) * x), None

        #self.density_head_block = Sdf2Densityv1() # NEURA TODO
        self.predictor = Sdf2Densityv1()

    # @torch.enable_grad()
    def forward(self, inputs, outputs=['sigma', 'texture']):
        if ('sigma' in outputs):
            inputs = super().forward(inputs, ['feat'])
            ...
            #inputs['sigma'] = self.density_head_block(inputs['sdf'])[0] # NEURA TODO
            inputs['sigma'] = self.predictor(inputs['sdf'])[0]
