...
class VolumeRenderer(Renderer):
    ...
    def __init__(self, args):
        ...
        self.random_eikonal_term = getattr(args, "random_eikonal_term", False)

    @staticmethod
    def add_args(parser):
        # ray-marching parameters
        parser.add_argument('--discrete-regularization', action='store_true',
                            help='if set, a zero mean unit variance gaussian will be added to encourage discreteness')
        ...
        parser.add_argument('--random-eikonal-term', action='store_true')


    def forward_once(self, input_fn, field_fn, ray_start, ray_dir, samples, encoder_states,
            early_stop=None,
            output_types=['sigma', 'texture']):
        """Compute the implicit field once given a set of rays.
        # Arguments
            ...
        # Returns
            outputs: dictionary
        ...
        sampled_depth = samples['sampled_point_depth'] # (num_rays, num_steps)
        sampled_idx = samples['sampled_point_voxel_idx'].long() # (num_rays, num_steps)
        ...
        sampled_xyz = ray(ray_start.unsqueeze(1), ray_dir.unsqueeze(1), sampled_depth.unsqueeze(2)) # (num_rays, num_steps, 3)
        sampled_dir = ray_dir.unsqueeze(1).expand(*sampled_depth.size(), ray_dir.size()[-1])
        samples['sampled_point_xyz'] = sampled_xyz # points in 3D xyz (num_rays, steps, 3)
        samples['sampled_point_ray_direction'] = sampled_dir # (num_rays, steps, 3)
    
        # apply mask    
        samples = {name: s[sample_mask] for name, s in samples.items()}
       
        # get encoder features as inputs, i.e., call fw() from JointVolumeEncoder,
        # compute the 3D points from intersections in cannonical space, add a deformation
        field_inputs = input_fn(samples, encoder_states)
        
        # forward implicit fields, e.g., call 
        ...
        if 'sigma' in field_outputs:
            ...
            noise = 0 if (not self.discrete_reg) or (not self.training) else torch.zeros_like(sigma).normal_()  
            ...
            outputs['free_energy'] = masked_scatter(sample_mask, free_energy) # (num_rays, steps)
        ...
        if 'joint_colors' in field_outputs:
            outputs['joint_colors'] = masked_scatter(sample_mask, field_outputs['joint_colors'])
        
        if hasattr(input_fn, "postcompute"):
            input_fn.postcompute(field_outputs, encoder_states)
        
        return outputs, sample_mask.sum()


    def forward_visibility(self, input_fn, field_fn, points_xyz, samples, encoder_states):
        return {}
    ...
    def forward_chunk(self, input_fn, field_fn, ray_start, ray_dir, samples, encoder_states,
            ...
            output_types=['sigma', 'texture'],
            global_weights=None):
        """Apply volumetric rendering in a chunk of rays.

        # Arguments
            input_fn: class, e.g., JointVolumeEncoder
            field_fn: class. e.g., RadianceField
            ray_start: (num_rays, 3)
            ray_dir: (num_rays, 3)
            samples: dictionary with
                'sampled_point_depth': tensor of depth values for sampling points on the ray (num_rays, num_steps+1)
                'sampled_point_distance': tensor with the distance between sampled points (num_rays, num_steps+1)
                'sampled_point_voxel_idx': tensor ??? (num_rays, num_steps+1)

            encoder_states: dictionary 
            gt_depths: not used? None
            output_types: list of strings
            global_weights: None

        # Returns (a dictionary with the fields)
            'probs': probability that this point hit a dense part, from [0,1] (num_rays, num_steps+1)
            'depths': expected depth, given the probability before: depth values for each ray (num_rays,)
            'max_depths': max depth of a ray that hist the underlying human mesh (num_rays,)
            'min_depths': min depth as before (num_rays,)
            'missed': float value in [0,1] indicating the changes that this ray missed a dense region, `1-probs.max(-1)` (num_rays)
            'ae': accumulated_evaluations, or dummy ?
            'colors': integrated color for each ray (num_rays, 3)
            'joint_colors': (num_rays, 4)
        ...
        sampled_depth = samples['sampled_point_depth'] # (num_rays, num_steps+1)
        sampled_idx = samples['sampled_point_voxel_idx'].long() # (num_rays, num_steps+1)
        original_depth = samples.get('original_point_depth', None) # None
        ...
        hits = sampled_idx.ne(-1).long() # (num_rays=1024, num_steps+1=64+1)
        ...
        if 'free_energy' in outputs:
            free_energy = outputs['free_energy'] # (num_rays, num_steps+1)
            shifted_free_energy = torch.cat([
                    free_energy.new_zeros(sampled_depth.size(0), 1),
                    free_energy[:, :-1]
                ], dim=-1)  # shift one step, (num_rays, num_steps+1)
            a = 1 - torch.exp(-free_energy.float())                             # probability of it is not empty here (num_rays, num_steps+1)
            b = torch.exp(-torch.cumsum(shifted_free_energy.float(), dim=-1))   # probability of everything is empty up to now (num_rays, num_steps+1)
            probs = (a * b).type_as(free_energy)                                # probability of the ray hits something here, (num_rays, num_steps+1)
        else:
        ...
        # It is not guaranteed that probs sums to one. The best is to use 'missed' values
        # with a very small threshold, around 0.0001 or less, then sampling for depth
        # or surface points during inference.
        missed = (1 - probs.sum(-1)) # Chance that a ray did not intersect anything dense (num_rays,)

        # For the non-missed rays, re-normalize probs, if in validation or testing
        # print (f'\n\nDEBUG>>>>> here the model is training=', self.training)
        if not self.training:
            valid = missed.lt(0.5)[..., None].to(dtype=probs.dtype).contiguous()
            probs = valid * (probs / probs.sum(-1, keepdim=True).clamp(0.01)) + (1 - valid) * probs

        # Compute expected depth values
        depth = (sampled_depth * probs).sum(-1) # Expected depth values given the hit probabilities (num_rays,)
        ...
        if 'normal' in outputs:
            ...
            if (not self.random_eikonal_term) or (not self.training):
                results['eikonal-term'] = (outputs['normal'].norm(p=2, dim=-1) - 1) ** 2
                results['eikonal-term'] = results['eikonal-term'][sampled_idx.ne(-1)]
            
            else:
                # randomly sample points to compute eikonal loss
                assert hasattr(input_fn, "generate_random_samples")
                random_points = input_fn.generate_random_samples(5 * ray_dir.numel())
                random_normal = field_fn(random_points, outputs=['sigma', 'normal'])['normal']
                results['eikonal-term'] = (random_normal.norm(p=2, dim=-1) - 1) ** 2
                
        if 'feat_n2' in outputs:
            results['feat_n2'] = (outputs['feat_n2'] * probs).sum(-1)
            results['regz-term'] = outputs['feat_n2'][sampled_idx.ne(-1)]
            
        if 'joint_colors' in outputs:
            results['joint_colors'] = (outputs['joint_colors'] * probs.unsqueeze(-1)).sum(-2)
