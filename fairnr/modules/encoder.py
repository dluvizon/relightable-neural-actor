...
class VolumeEncoder(Encoder):
    ...
    def ray_sample(self, intersection_outputs):
        ...
        return {
            'sampled_point_depth': sampled_depth, # (1024, 64+1)
            'sampled_point_distance': sampled_dists, # (1024, 64+1)
            'sampled_point_voxel_idx': sampled_idx,  # dummy index (to match raymarcher)... all zeros? (1024, 64+1)
        }

    def forward(self, samples, encoder_states):
        # print (f'DEBUG:: call VolumeEncoder.forward()')
        inputs = {
...
class JointVolumeEncoder(VolumeEncoder):
    ...
    def __init__(self, args):
        ...
        #self.texture_encoder_fn = None # NEURA TODO
        ...
        if getattr(self.args, "texuv", None) is not None:  # this file saved uv coordinates of texture map
            ...
            vt, ft = read_obj(self.args.texuv) # vt: (7450, 2), ft: (13776, 3) TODO: why we have 7450 vertices here??
            ...
            if getattr(self.args, "use_texture_encoder", False):
                ...
                #self.texture_encoder_fn = SpatialEncoder( # NEURA TODO
        ...
        # print (f'DEBUG:: min_dis_eps', self.min_dis_eps)
        ...
        if self.texture_to_deformation:
            #if self.texture_encoder_fn is not None: # NEURA TODO
            if self.texture_encoder is not None:
                #input_dim += self.texture_encoder_fn.latent_size # NEURA TODO
        ...
        if self.additional_deform == 'pos':
            #self.skinning_deform_block = FCBlock( # NEURA TODO
            self.skinning_deform = FCBlock(
                self.model_dim, 0, input_dim, 3, # 256, 0, 512+256, 3
                outermost_linear=True, with_ln=False)
            #self.skinning_deform_block.net[-1].weight.data *= 0 # NEURA
            #self.skinning_deform_block.net[-1].bias.data *= 0 # NEURA
        ...
        if not self.disable_query_point:
            #self.merge_joint_field_block = FCBlock( # NEURA TODO
    ...
    def get_edge(self, ray_start, ray_dir, *args, **kwargs):
        # print (f'DEBUG:: call JointVolumeEncoder.get_edge()')
    ...
    def precompute(self, id=None, *args, **kwargs):
        """The main task of this function is to compute the texture features
        (in variable texture_map), which are image features computed from the input
        texture UV map that is taken as input. It also computes 

        # Arguments
            id: dummy? not used
            args: dummy? not used
            kwargs: dictionary with (used) keys
        ...
        # Returns (as a dictionary)
            joints:     torch.Size([1, 24, 3])
            joints_RT:  torch.Size([1, 24, 16])
            R:          torch.Size([1, 3, 3])
            T:          torch.Size([1, 1, 3])
            joints_pose:    torch.Size([1, 1, 72])
            new_joints_RT:  torch.Size([1, 24, 16])
            texture_map[0]: torch.Size([1, 512, 256, 256])
            texture_rgb[0]: torch.Size([1, 3, 512, 512])
            vertices:       torch.Size([1, 6890, 3]), posed vertices, without global rotation and translation, i.e., normalized posed space
            joint_normalized:   torch.Size([1, 24, 3]), 3D joints in normalized posed space
        """
        # prepare the encoder before volume rendering
        # print (f'DEBUG:: call JointVolumeEncoder.precompute()!!! id=', id)

        assert id.size(0) == 1, 'only one shape per batch'    
        encoder_states = {
            "joints": kwargs["joints"],        # joint locations (B, J=24, 3)
            "joints_RT": kwargs["joints_RT"].permute(0,3,1,2).reshape(1,-1,16),  # joint RT (B, J, 16)
            "R": kwargs["rotation"],           # global rotation (B, 3, 3)
            "T": kwargs["translation"],        # global translation (B, 1, 3)
            "joints_pose": kwargs["pose"]      # joint feature (B, 1, 72)
        }
        if self.tpose_data is not None:
            encoder_states['new_joints_RT'] = torch.tensor(self.tpose_data['joints_RT']).type_as(
                encoder_states['joints_RT']).permute(2,0,1).reshape(1,-1,16) # (B, J, 16)

        if (self.face_uv is not None) and (not self.disable_texture):
            texture_map = kwargs['tex'].permute(0, 3, 1, 2)  # 1 x [RGB]=3 x 512 x 512
            if self.texture_mask is not None:
                texture_map = texture_map * torch.sigmoid(self.texture_mask[None, None, :,:])
            texture_rgb = torch.flip(texture_map, [2])    # texture rgb # Why vflip??

            # if self.texture_encoder_fn is not None: # NEURA TODO
            #     texture_map = self.texture_encoder_fn(texture_map)
            if self.texture_encoder is not None:
                texture_map = self.texture_encoder(texture_map)
            texture_map = torch.flip(texture_map, [2]) 

            encoder_states['texture_map'] = [texture_map] # list of one tensor with shape (batch, 512, 256, 256)
            encoder_states['texture_rgb'] = [texture_rgb] # list of one tensor with shape (batch, 3, 512, 512)

        # get the current mesh, mesh lays in normalized space.
        A = encoder_states["joints_RT"].squeeze(0) # shape (24, 16)
        weighted_RT = (self.weights @ A).reshape(-1, 4, 4) # self.weights=(num_vertices=N, 24), N x 4 x 4
        # >> N x 4 (normalized space, i.e., cannonical space)
        vertices = torch.cat([self.vertices, self.vertices.new_ones(self.vertices.size(0), 1)], -1)
        vertices = torch.einsum("ncd,nd->nc", weighted_RT, vertices)[:, :3]   #  (normalized space) -> posed?
        encoder_states["vertices"] = vertices.unsqueeze(0) # (B, V=6890, 3)
        ...
        encoder_states["joint_normalized"] = joints_normalized.unsqueeze(0) # (B, J, 3)
    ...
    def ray_intersect(self, ray_start, ray_dir, encoder_states):
        """This function intersects the rays with the underlying human mesh.

        # Arguments
            ray_start: tensor shape (batch, 1, 1, 3)
            ray_dir: tensor shape (batch, 1, num_rays, 3)
            encoder_states: dictionary (from JointVolumeEncoder.precompute), used keys:
                'joints': 3D human joints with shape (batch, 24, 3)
                'vertices': human mode vertices in normalized posed space (batch, 6890, 3)
                'R': global human rotation with shape (batch, 3, 3)
                'T': global human translation with shape (batch, 1, 3)
                'texture_rgb': texture map from where pixel values are read based on the UV coordinates (batch, 3, 512, 512)

        # Returns
            ray_start: (batch, num_rays, 3)
            ray_dir:  (batch, num_rays, 3)
            intersection_outputs: dictionary with
                'min_depth': (batch, num_rays, 1)
                'max_depth': (batch, num_rays, 1)
                'probs': dummy ones (batch, num_rays, 1)
                'steps': dummy (batch, num_rays)
                'intersected_voxel_idx': dummy zeros (batch, num_rays, 1)
                'mesh_hits': boolean (batch, num_rays, 1)
                'mesh_depth': intersection depth value (batch, num_rays)
                'mesh_color': intersection RGB color (interpolated from tex map) shape (batch, num_rays, 3)
            hits: boolean tensor with shape (batch, num_rays)

        """
        # print (f'DEBUG:: call JointVolumeEncoder.ray_intersect()')
        S, V, P, _ = ray_dir.size()

        # ray-voxel intersection, expand the starting ray if needed
        _, sV, sP, _ = ray_start.size()
        if sV * sP != V * P:
            ray_start = ray_start.expand_as(ray_dir).contiguous().view(S, V * P, 3).contiguous() # (batch=S, num_rays=P, 3)
        else:
            ray_start = ray_start.reshape(S, V * P, 3).contiguous()
        ray_dir = ray_dir.reshape(S, V * P, 3).contiguous() # (S, num_rays, 3)

        center = encoder_states["joints"][0].mean(0, keepdim=True) # average of all body joints (batch, 3)
        radius = (encoder_states["joints"][0] - center).norm(2, -1).max() # farthest joint from the center (1,)
        radius = radius * 1.88 # This defined the expected ratio of a sphere where the human is enclosed

        # we intersect the bounding sphere
        h_cet = center[None, :, :] - ray_start # (batch, num_rays, 3)
        h_mid = (h_cet * ray_dir).sum(-1) # (batch, num_rays)
        h_dis = (h_cet ** 2).sum(-1) - h_mid ** 2 # == closest_point_to_the_center ** 2
        hits = h_dis < (radius ** 2) # boolean maks (batch, num_rays)

        # select only te rays that are intersecting the human sphere
        hit_ray_start = ray_start[hits]  # M x 3, where M=sum(hits)
        hit_ray_dir = ray_dir[hits]      # M x 3

        # vertices from SMPL in normalized posed space, i.e., without rotation and translation, but posed
        vertices_normalized = encoder_states["vertices"][0]
        vertices = torch.matmul(vertices_normalized, encoder_states["R"][0]) + encoder_states["T"][0]  # N x 3, mesh in posed global space

        # -------------------- rendering mesh ----------------------------------- #
        mesh_depth = h_mid.clone() # TODO: Why the middle point is kept as depth for background???
        mesh_hits = hits.clone() # Keep those hits to map it back later
        ...
        # pts_idx index of hitted FACES has shape (batch, M, n_max=100)
        # depth00 has shape (batch, M, n_max=100, 3), where depth00[..., 1:3] are all zeros!
        # uv has shape (batch, M, 200), values from 0 to 1
        hit_mesh = pts_idx.ne(-1).any(-1) # boolean with shape (batch, M) indicates whether at least one triangle was hit

        depth000, index000 = depth00[:,:,:,0].masked_fill_(pts_idx.eq(-1), MAX_DEPTH).min(-1) # take the first hits in depth, shape (batch, M)
        mesh_depth[hits]   = depth000 # shape (batch, num_rays)
        mesh_hits[hits]    = hit_mesh # shape (batch, num_rays)

        if (getattr(self, 'face_uv', None) is not None) and (not self.no_textured_mesh):
            select000 = pts_idx.gather(2, index000.unsqueeze(-1)).squeeze(-1) # index of the selected faces (batch, M)
            uv = uv.reshape(*uv.size()[:-1], -1, 2) # shape (batch, M, n_max=100, 2)
            u000, v000 = uv[..., 0], uv[..., 1]
            u000 = u000.gather(2, index000.unsqueeze(-1)).squeeze(-1)
            v000 = v000.gather(2, index000.unsqueeze(-1)).squeeze(-1)
            select000 = select000[hit_mesh] # indices of the faces hitted by a ray, shape (num_hits,), where `num_hits <= M`
            u000 = u000[hit_mesh] # indices UV in [0,1] of the rays for each hitted face
            v000 = v000[hit_mesh]
            w000 = 1 - u000 - v000
            bary_coords = torch.stack([u000, v000, w000], 1) # shape (num_hits, 3)
            # bary_coords = torch.stack([w000, u000, v000], 1) # shape (num_hits, 3)

            sampled_uvs = (self.face_uv[select000.long()] * bary_coords.unsqueeze(-1)).sum(1) # (num_hits, 2)
            if getattr(self.args, "uv_textured_mesh", False):
                ...
                grids = sampled_uvs[None, :, None, :] * 2 - 1 # (1, num_hits, 1, 2)
                texture_rgb = encoder_states['texture_rgb'][0] # shape (batch, 3, 512, 512)
                textures = F.grid_sample(texture_rgb, grids, mode='bilinear', align_corners=False) # sampled textures (batch, 3, num_hits, 1)
                sampled_tex = textures.permute(0, 2, 3, 1).reshape(1, -1, 3) # (batch, num_hits, 3)
            
            mesh_color000 = mesh_depth.new_ones(*depth000.size(), 3) # (batch, num_hits, 3)
            mesh_color000[hit_mesh] = sampled_tex
            
            mesh_color = mesh_depth.new_ones(*mesh_depth.size(), 3) # (batch, num_rays, 3)
        ...
        min_depth, max_depth = h_mid.clone(), h_mid.clone() # (B, num_rays)
        hits = hits.masked_scatter(hits, hit_mask) # (B, num_rays)
        min_depth[hits] = hit_min_depth[hit_mask] # (B, num_rays)
        max_depth[hits] = hit_max_depth[hit_mask] # (B, num_rays)
    ...
    def forward(self, samples, encoder_states=None):
        ...
        # Arguments
            samples: dictionary with fields: 'sampled_point_xyz', 'sampled_point_depth',
                'sampled_point_distance', etc...
            encode_states: dictionary with fields: 'joints', 'joints_RT', 'R', 'T',
                'joints_pose', 'new_joints_RT', 'texture_map', 'texture_rgb',
                'vertices', 'joint_normalized'
        ...
        # Returns
            A dictionary with the fields:
                ...
                ...
        """
        # print (f'DEBUG:: call JointVolumeEncoder.forward()!!!', samples.keys())

        sampled_dir = samples.get('sampled_point_ray_direction', None) # (N, 3)
        sampled_dis = samples.get('sampled_point_distance', None) # (N,)
        ...
        if encoder_states is not None:  # attention with joints
            ...
            vertices = encoder_states['vertices'].contiguous()   # mesh (vertices) in normalized posed space (6890, 3), 

            from fairnr.clib._ext import point_face_dist_forward

            triangles = F.embedding(self.faces, vertices) # (13776, 3, 3)
            ...
            # bary_coords = torch.stack([w2, w0, w1], 1)   # B x 3
            sampled_uvs = (
                self.face_uv[min_face_idx] # (num_rays, 3, 2)
                    * bary_coords.unsqueeze(-1) # (num_rays, 3, 1)
                    ).sum(1) # (num_rays, 2)

            if self.use_local_coordinate:
                # compute triangle normals
                A, B, C = triangles[:,0], triangles[:,1], triangles[:,2]
                triangle_normals = torch.cross((B - A), (C - B), 1) # (num_rays, 3)
                faces_xyzs = (triangles[min_face_idx] * bary_coords.unsqueeze(-1)).sum(1) # (num_rays, 3)
                insideout = ((sampled_normalized - faces_xyzs) * triangle_normals[min_face_idx]).sum(-1).sign() #  [-1, 1] (num_rays,)
                local_coords = torch.cat([
                        sampled_uvs * 2 - 1, # (num_rays, 2)
                        (min_dis.sqrt() * insideout)[:, None] # (num_rays, 1)
                    ], 1) # (num_rays, 3)
            
            if (self.face_uv is not None) and (not self.disable_texture):
                texture_map = encoder_states['texture_map'][0]  # query texture map here, shape (batch, 512, 256, 256)
                f_dim = texture_map.size(1)
                grids = sampled_uvs[None, :, None, :] * 2 - 1   # UV coords normalized from [-1, 1] with shape (1, num_rays, 1, 2)
            ...
            if self.use_local_coordinate:
                x = self.joint_filter(local_coords) # positional encoder for local joint coords (u,v,h) with shape (num_rays, 39)
            ...
            if not self.disable_query_point:
                #attnout = self.merge_joint_field_block(x) # NEURA TODO
                attnout = self.merge_joint_field(x)
            inputs["attnout"] = attnout # positional encoding after FC layer, shape (num_rays, 256)
            ...
            if not self.disable_projection:   # project to canonical space?
                ...
                inputs["weights"] = weights # (num_rays, 24)
                inputs["joint_colors"] = weights @ self.joint_colors.type_as(weights) # (num_rays, 4)
                inputs["posed_pos"] = sampled_normalized    # sampled points in the normalized posed space, not global space (num_rays, 3)

                A = encoder_states["joints_RT"] # (24, 16)
                ...
                sampled_canonical = torch.einsum("ncd,nd->nc", torch.inverse(weighted_RT), sampled_normalized) # points in cannonical space (num_rays, 4)
                ...
                if self.tpose_data is not None:   # A custom T-Pose is given for this sample
                ...
                sampled_canonical = sampled_canonical[:, :3] # Final points in the cannonical space, i.e., unposed, shape (num_rays, 3)
    
                if self.additional_deform == "pos": # add an additional 3D deformation to individual points
                    if not self.texture_to_deformation:
                        ...
                        if not self.disable_query_point:
                            deform_input = torch.cat([attnout, sampled_tex], -1) # (num_rays, 256+512)
                        else:
                            deform_input = sampled_tex
                    
                    #sampled_canonical = sampled_canonical + self.skinning_deform_block(deform_input) # add deformation (from a FCNet) to the cannonical point # NEURA TODO
                    residual_deform = self.skinning_deform(deform_input)
                    sampled_canonical = sampled_canonical + residual_deform
                inputs["pos"] = sampled_canonical # (num_rays, 3)
                ...
                if self.additional_deform == "pos":
                    if not self.texture_to_deformation:
                        ...
                        if not self.disable_query_point:
                            ...
                            deform_input = sampled_tex      
                    #samples = samples + self.skinning_deform_block(deform_input) # NEURA TODO
