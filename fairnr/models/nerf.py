...
class NeRFModel(BaseModel):
    ...
    def intersecting(self, ray_start, ray_dir, encoder_states, **kwargs):
        """Wrapper to the function `ray_intersect()` from the encoder"""
        # print (f'DEBUG:: call NeRFModel.intersecting')
...
def base_architecture(args):
    ...
    args.near = getattr(args, "near", 1)
    args.far = getattr(args, "far", 10)
...
class JointNeRFModel(NeRFModel):
    ...
    def raymarching(self, ray_start, ray_dir, intersection_outputs, encoder_states, fine=False):
        ...
        # Arguments
            ray_start: torch tensor with shape (N, 3)
            ray_dir: torch tensor with shape (N, 3)
            intersection_outputs: dictionary
                'min_depth' and 'max_depth': tensor with shape (N, 1)
                'probs': tensor with shape (N, 1)
                'steps': tensor with shape (N,)
                'intersected_voxel_idx': tensor with shape (N, 1)
                'mesh_hits': (N,)
                'mesh_depth': (N,)
                'mesh_color': (N, 3)
            encoder_states: 
        ...
        # Returns
        """
        # print ('DEBUG:: call NeRFModel.raymarching()')
    ...
    def postprocessing(self, ray_start, ray_dir, all_results, hits, sizes):
        ...
        S, V, P = sizes
        fullsize = S * V * P
        BG_DEPTH = 0 # self.field.bg_color.depth
        ...
        if 'missed' in all_results:
            ...
            all_results['max_depths'] = fill_in((fullsize, ), hits, all_results['max_depths'],
                all_results['max_depths'].max(), device).view(S, V, P)
            all_results['min_depths'] = fill_in((fullsize, ), hits, all_results['min_depths'],
                all_results['min_depths'].min(), device).view(S, V, P)

            num_spls = all_results['probs'].size(-1)
            all_results['probs'] = fill_in((fullsize, num_spls), hits, all_results['probs'], 1e-7, device).view(S, V, P, num_spls)

            all_results['xyz'] = ray_start[None, None] + ray_dir[None, None] * all_results['depths'][..., None]

            if 'lvis' in all_results:
                num_envpix = all_results['lvis'].size(-1)
                all_results['lvis'] = fill_in((fullsize, num_envpix), hits, all_results['lvis'], 0.0, device).view(S, V, P, num_envpix)
        ...
        if 'voxel_masks' in all_results:
            ...
            all_results['voxel_masks'] = fill_in((fullsize, ), hits, all_results['voxel_masks'], False, device).view(S, V, P)
...
def joint_nerf_coarse_architecture(args):
    # print (f'DEBUG:: enter joint_nerf_coarse_architecture')
